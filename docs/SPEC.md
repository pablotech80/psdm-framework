...
- Provide a dependency-free read-only interactive shell with `/help`, `/status`, `/audit`, `/inspect`, and `/exit` routing.
...
- `node bin/psdm.mjs shell <target>` renders target-specific context and routes `/help`, `/status`, `/audit`, `/inspect`, and `/exit` through an allowlist.
...
- The slash palette exposes only `/help`, `/status`, `/audit`, `/inspect`, and `/exit`; navigation wraps and filtered selection executes through the existing command router.
