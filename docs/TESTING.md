...
node bin/psdm.mjs review "add Google OAuth login" --staged --files src/auth/login.mjs,tests/auth/login.test.mjs --json
...
- Verify shell validation matches the existing validator decision, prioritizes failing artifacts, and creates no target files.
...
- Run Decision Review fixtures for aligned scope, files outside scope, expected files missing from staged state, unexpected delivery surfaces, dependency deltas, validation non-claims, and authority-unverified wording.
